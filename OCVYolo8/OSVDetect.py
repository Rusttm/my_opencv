...
class OSVDetect(OCVMainClass):
    ...
    def change_out_file(self, file_name: str = None):
        ...
        try:
            while True:
                ...
                for result in results:
                    ...
                    # capture tail part
                    time_passed = person_captured_last_time + datetime.timedelta(seconds=person_captured_time_delay)
                    is_time_passed = datetime.datetime.now() > time_passed
                    # check is person captured during last 3 sec?
                    if is_time_passed and person_captured:
                        person_captured = False
                        person_captured_last_time = datetime.datetime.now()
                        print("capture resets")
                    ...
                    for box in boxes:
                        ...
                        if cls != 0:
                            ...
                            # print(f"{is_time_passed=} and {person_captured=}")
                            if not is_time_passed and person_captured:
                                self._out.write(img)
                                print(f"tail video writing to {self._file_name}")
                            else:
                                person_captured = False
                                person_captured_last_time = datetime.datetime.now()
                            continue
